...
class ReportAScore: UIViewController {
    ...
    @objc fileprivate func submitHit(){
        ...
        Firestore.firestore().collection("Scores").document(self.currentUsr1).collection("GameScores").document().setData(["player1" : uid, "player2": currentUsr1, "player1Score": myArrayOfIntsp2, "player2Score": myArrayOfIntsp1, "timestamp" : date, "gamesP1":      gamesForP2, "gamesP2": gamesForP1])
